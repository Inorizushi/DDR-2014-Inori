...
function LoadPlayerStuff(Player)
	...
	t[#t+1] = LoadActor( THEME:GetPathG("ScreenSelectProfile","CardBG02"))..{
		InitCommand=cmd(y,-144;zoomx,1;diffusealpha,0);
		OnCommand=function(self)
			self:sleep(0.8):linear(0.3):diffusealpha(1)
		end;
	...
	t[#t+1] = LoadActor( THEME:GetPathG("ScreenSelectProfile","DancerNameWord") ) .. {
		InitCommand=cmd(shadowlength,0;zoom,0;x,-104;y,-238);
		OnCommand=function(self)
			self:sleep(0.7):y(-238):linear(0.3):zoom(1)
		end;
		OffCommand=cmd(stoptweening;linear,0.02;zoom,0.100;diffusealpha,0);
	};
	t[#t+1] = LoadActor( THEME:GetPathG("ScreenSelectProfile","DDR-CodeWord") ) .. {
		InitCommand=cmd(shadowlength,0;zoom,0;y,-160;x,-112);
		OnCommand=function(self)
			self:sleep(0.7):linear(0.3):zoom(1)
		end;
		OffCommand=cmd(stoptweening;linear,0.02;zoom,0.100;diffusealpha,0);
	};
	t[#t+1] = LoadActor( THEME:GetPathG("ScreenSelectProfile","LvBackground") ) .. {
		InitCommand=cmd(shadowlength,0;zoom,0;x,-142;y,-94);
		OnCommand=cmd(sleep,0.7;linear,0.3;zoom,1.3;);
		OffCommand=cmd(linear,0.02;zoom,0.100;diffusealpha,0);
	};
  ...
  InitCommand=cmd(x,-142;y,-94;zoom,0;strokecolor,color("#000000");maxwidth,50);
    ...
    OnCommand=function(self)
      self:sleep(0.7):linear(0.3):diffusealpha(1)
    end;
		OffCommand=cmd(stoptweening;linear,0.1;zoomy,0;diffusealpha,0);
	};
	...
	if (Player == PLAYER_1) then ---------------P1
		...
		if GAMESTATE:GetCurrentStyle():GetStepsType() == "StepsType_Dance_Single" then
			t[#t+1] = LoadActor( THEME:GetPathG("ScreenDataSaveSummary", "GrooveRadar" ),0,0,0,0,0,1,0,PLAYER_1)..{  --00000/Style/IsPastValue ::  Single=1 Double=2 IsPastValue=0 or 1
				Name = "GVRD1";
				InitCommand=cmd(x,0;y,99;zoom,1;diffusealpha,0;diffuse,Color("Yellow"));
...
x[#x+1] = Def.ActorFrame {
			Name = 'P1Frame';
			InitCommand=cmd(x,SCREEN_CENTER_X-160;y,SCREEN_CENTER_Y+20;zoom,1);
			OnCommand=function(self)
